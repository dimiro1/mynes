 ...
 * <p>
 * <b>Nothing here touches {@code mynes.ui}, and nothing here can:</b> this is a module of its own,
 * and {@code mynes-desktop} is not on its class path. Three separate things over there would make a
 * run depend on the computer it ran on: the look and feel loads a native library, {@code
 * AudioOutput} opens a mixer, and {@code Config} reads whatever is in the user's home directory. A
 ...
 * <p>
 * The palettes themselves used to be the one exception to that sentence. They are in
 * {@link com.github.dimiro1.mynes.palette} now, which is in the core, so there is no exception left
 * to make.
 */
